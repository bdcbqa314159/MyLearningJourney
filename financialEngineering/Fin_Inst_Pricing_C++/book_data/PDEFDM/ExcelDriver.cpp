...
private:
	...
	void ToSheetHorizontal( Excel::_WorksheetPtr sheet,
					...
					string label,
					const Vector<double, long> & values )
	{
		// Get cells.
	...
	Vector <double, long> createVector(const Matrix<double,long>& mat, long row)
	{
		long nCols = mat.Columns();

		Vector<double, long> result (nCols, mat.MinColumnIndex());
...
public:
	...
	void CreateChart(const Vector<double, long> & x, const list<string> & labels, const list<Vector<double, long> > & vectorList, 
	...
	//  xTitle:		label of x axis
	//  yTitle:		label of y axis
	void CreateChart(const Vector<double, long> & x, const Vector<double, long> & y, const std::string& chartTitle, const std::string& xTitle = "X", const std::string& yTitle = "Y")
	{ 
	...
	void AddMatrix(string name, const NumericMatrix<double, long> & matrix, const list<string> & rowLabels, const list<string> & columnLabels)
	{
	...
	void GetPropertySet(SimplePropertySet<string, double> & pset)
	{
		...
		Excel::_WorksheetPtr pSheet = pWorkbook->Worksheets->GetItem(nname.c_str());
//	Excel::_WorksheetPtr pWorksheet=pWorkbook->Worksheets->GetItem(name.c_str()); OLD and GOOD
		...
		while(true)
		{
			...
			long tt = 1;
			_bstr_t bstrName = pRange->Item[row][tt]; 
	//		string name = (string)(pRange->Item[row][1]);
			...
			if( name != string("") )
			{
				...
				tt = 2;
				double value = (double)pRange->Item[row][tt];
				pset.set(name, value);
			}
			else
			{
